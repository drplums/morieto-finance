...
def get_trending():
    trending = R.get_json("https://api.stocktwits.com/api/2/streams/trending.json", params=ST_BASE_PARAMS)
    return trending

def get_trending_stocks():
    """ returns list of trending stock symbols, ensuring each symbol is part of a NYSE or NASDAQ
    """
    trending = R.get_json(ST_BASE_URL + 'trending/symbols.json', params=ST_BASE_PARAMS)
    trending = trending['symbols']
    # symbols = [s['symbol'] for s in trending if s['exchange'] in EXCHANGES]
    symbols = [(s['symbol'], s['title'])for s in trending]
    return symbols
...
if __name__ == '__main__':
    ...
    pp.pprint(get_trending_stocks())
    #pp.pprint(get_stock_stream('aapl'))
